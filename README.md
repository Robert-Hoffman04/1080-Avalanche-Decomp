...
- `config/[GTEE01]` - Configuration files for each game version.
- `config/[GTEE01]/build.sha1` - SHA-1 hashes for each built artifact, for final verification.
- `build/` - Build artifacts generated by the the build process. Ignored by `.gitignore`.
- `orig/[GTEE01]` - Original game files, extracted from the disc. Ignored by `.gitignore`.
- `orig/[GTEE01]/.gitkeep` - Empty checked-in file to ensure the directory is created on clone.
...
- `config/GTEE01/config.example.yml` - Example configuration file and documentation.

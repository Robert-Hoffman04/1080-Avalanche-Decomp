...
2. Rename `orig/GTEE01` to the game's ID. (For example, `GLZE01` for _The Legend of Zelda: The Wind Waker_.)

3. Extract your game to `orig/[GTEE01]`. In Dolphin, use "Extract Entire Disc" for GameCube games, or use "Data Partition" -> "Extract Entire Partition" for Wii games.

4. Rename `config/GTEE01` to the game's ID and modify `config/[GTEE01]/config.yml` appropriately, using [`config.example.yml`](/config/GTEE01/config.example.yml) as a reference. If the game doesn't use RELs, the `modules` list in `config.yml` can be removed.

5. Generate a `config/[GTEE01]/build.sha1` file for verification. This file is a list of SHA-1 hashes for each build artifact. One possible way:

    ```sh
    dtk shasum orig/[GTEE01]/sys/main.dol orig/[GTEE01]/files/*.rel -o config/[GTEE01]/build.sha1
    ```

6. Modify the paths in `config/[GTEE01]/build.sha1` to point to the `build` directory instead of `orig`. The DOL will be built at `build/[GTEE01]/main.dol`, and modules will be built at `build/[GTEE01]/[module_name]/[module_name].rel`.
...
Add the `map` key to `config.yml`, pointing to the `.map` file from the game disc. (For example, `orig/[GTEE01]/files/main.map`.) For RELs, add a `map` key to each module in `config.yml`.

If the game uses [common BSS](common_bss.md), be sure to set `common_start` as well. (See [`config.example.yml`](/config/GTEE01/config.example.yml).) Otherwise, the final link order may fail to be determined.
